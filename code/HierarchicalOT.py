...
def sparse_sinkhorn(weights1, weights2, M,eps=1,method="sinkhorn",numItermax=1e3,log=False):
    """ Compute entropic transport for (posssibly) un-normalized sparse distributions"""
    ...
    weights_2_active = weights2[active2]
    M_reduced = np.ascontiguousarray(M[active1][:,active2]) 
    # cost matrix where weights is not 10
    
    return ot.sinkhorn2(weights_1_active,weights_2_active,M_reduced,
    reg=eps,method=method,numItermax=numItermax,log=log)
