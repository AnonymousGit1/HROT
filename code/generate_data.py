...
def make_data_1d(expe,n_samples,device='cpu'):
    if expe=="cluster":
        ...
        xs=np.concatenate((np.random.normal(-3, 1, 2*n_samples),np.random.normal(15, 1, n_samples)))
        xt=np.concatenate((np.random.normal(0, 1, 3*n_samples),np.random.normal(-10, .05, int(n_samples/30))))
